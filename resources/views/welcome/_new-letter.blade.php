            ...
            <div class="md:flex">
                ...
                <div id="alreadySubscribedSection" class="md:w-1/2 p-8 bg-indigo-700 text-white">
                    ...
                    <div class="text-center">
                        <form onsubmit="return confirm('Voulez-vous vraiment effectuer cette action ?')" action="{{ route('news-letter.remove') }}" method="post">
                            @csrf
                ...
                <div id="subscribeSection" class="md:w-1/2 p-8 fade-in">
                    ...
                    <form onsubmit="return confirm('Voulez-vous vraiment effectuer cette action ?')" id="subscribeForm" class="space-y-4" action="{{ route('news-letter.add') }}" method="post">
                        <input type="email" placeholder="Votre adresse email" required
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-300" value="{{ Auth::user()->email }}">
                        @csrf
                        <button type="submit"
